...
impl ServerCertVerifier for NoCertificateVerification {
    // Cast devices present self-signed certificates, and many of them (notably
    // AV receivers with Chromecast built-in) generate X.509 version 1
    // certificates. The rustls signature-verification helpers parse the
    // certificate with webpki to extract the public key, and webpki only
    // supports version 3 certificates, so delegating rejects those devices
    // during the handshake with `invalid peer certificate:
    // UnsupportedCertVersion` (https://github.com/rustls/webpki/issues/29).
    // This verifier performs no validation by contract, so the signature
    // checks assert validity instead, matching the pattern of the rustls
    // `danger` verifier examples.
